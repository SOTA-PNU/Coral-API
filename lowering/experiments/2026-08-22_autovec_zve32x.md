...
## 추가: data-tiling + im2col (2026-08-23)

사용자 제안 조합을 올바른 피처(+zve32x,+zvl128b)로 재실험:

| 빌드 (lenet5) | 사이클 | 정확도 |
|---|---|---|
| 스칼라 | 2,650,300 | ✓ |
| +zve32x,+zvl128b | 2,032,503 | ✓ 비트 동일 |
| 위 + `--iree-opt-data-tiling --iree-global-opt-use-im2col-for-convs=true` | **3,790,483 (+86% vs 벡터)** | ✓ 비트 동일 |

교훈: im2col 은 5x5 conv 입력을 ~25배로 복제해 명령 수(=이 시뮬의 사이클)를
크게 늘리고, data-tiling 의 pack/unpack 도 이를 소비할 ukernel 없이는 순수
오버헤드다. 타일링 플래그는 (a) 타일 레이아웃을 먹는 ukernel, (b) 실제 메모리
계층 모델(RTL) 위에서만 의미가 있다 — 단독으로는 역효과.
